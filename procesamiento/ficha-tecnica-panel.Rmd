...
* **Ola 1**:
  * Inicio: `r as.Date(min(data_w01$StartDate))`
  * Término: `r as.Date(max(data_w01$EndDate))`

* **Ola 2**: 
  * Inicio: `r as.Date(min(data_w02$StartDate))` 
  * Término: `r as.Date(max(data_w02$EndDate))`

* **Ola 3**:
  * Inicio: `r as.Date(min(data_w03$StartDate))`
  * Término: `r as.Date(max(data_w03$EndDate))`
:::
::::::
...
* si deseamos vincular las bases de datos podemos emplear el siguiente código:
...
data_wide <- 
  left_join(data_w01b,data_w02b, by = "folio_encuestado") %>% 
  left_join(data_w03b, by = "folio_encuestado")

# Revisar consistencia en variable sexo
data_wide[data_wide==-999] <- NA # declarar NA
sjPlot::sjp.corr(data_wide[,c("cs_genero_w01","cs_genero_w02","cs_genero_w03")])
```
