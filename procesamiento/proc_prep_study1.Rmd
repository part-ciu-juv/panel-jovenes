...
# PSU
study_1$psu <- set_label(study_1$psu,"University Selection Test (PSU)")
...
vars <- study_1 %>% select(firm1a:segu5a) %>% names()
...
varsvote <- study_1 %>% select(votpr17,votpleb,votcons) %>% names()
...
varsvote <- study_1 %>% select(votconv:vot2vue) %>% names()
study_1[,varsvote]<- lapply(study_1[,varsvote], ordered)
valab <- c("No","Yes")
p2<- c(
  ...
  "Vote: Presidential elections - first round (2020)",
  "Vote: Presidential elections - second round (2020)")
etiquetas <- p2
...
vars2 <- study_1 %>% select(conv1a:marc5a) %>% names()
...
study_1 %>% select(starts_with("pospol")) %>% sjmisc::frq()

df <- study_1 %>% select(psu,educyear,edmadyear,edpadyear,starts_with("onli")) %>% na.omit()
# skimr::skim(df)
