...
load(here::here("input/data/proc/study1.RData"))
data1 <- study_1 %>% dplyr::select(-educ,-edmad,-edpad,-edpadyear,-edmadyear,-edparavg) %>% na.omit()
...
m1a<- '
        ...
        # efecto directo
        disrup1a ~ C1*edhigh+B1A*educyear+B1B*conv1a+mujer+pospolcen+pospolizq+pospolind
        ...
        # efecto directo
        online1a ~ C2*edhigh+B2A*educyear+B2B*conv1a+mujer+pospolcen+pospolizq+pospolind
...
```{r part-edhigh-psu, results='asis'}
rm(list=ls())
if (!require("pacman")) install.packages("pacman") # instalar pacman
pacman::p_load(dplyr,       # Manipulacion de datos 
               haven,       # importar datos en .dta o .sav
...
m1a<- '
        ...
        # (1) modelo de regresion: PARTICIPACION DISRUPTIVA ____________________
        # efecto directo
        disrup1a ~ C1*edhigh+B1A*psu+B1B*conv1a+mujer+pospolcen+pospolizq+pospolind
        
        # efecto mediacion predictor sobre mediador (DECLARAR UNA VEZ)
        # educ padres sobre educacion hijos
        psu ~ A1A*edhigh+mujer+pospolcen+pospolizq+pospolind
        ...
        # (2) modelo de regresion: PARTICIPACION ONLINE ________________________
        # efecto directo
        online1a ~ C2*edhigh+B2A*psu+B2B*conv1a+mujer+pospolcen+pospolizq+pospolind
...
kable(tabdis,format = "html",
      booktabs=T,
      row.names = F,
      caption = "Disruptive political participation, PSU scores and conversation",
      col.names = col
      ) %>% 
  kable_styling(full_width = F)
tab_dis_psu
...
kable(tabonl,format = "html",
      booktabs=T,
      row.names = F,
      caption = "Online political participation, PSU scores and conversation",
      col.names = col
      ) %>% 
  kable_styling(full_width = F) %>% 
  row_spec(7,hline_after = T)
tab_onl_psu
tables2 <- list(tab_dis_psu,tab_onl_psu)
save(tables2,
     file = here::here("output/tables/tab_psu.RData"))
```
...
# Hypothesis___________________________________________________________________
m1 <- "edhigh+psu+conv1a+mujer+pospol"#highschool
m2 <- "edhigh+psu+conv3a+mujer+pospol"# 2sem 2020
m3 <- "edhigh+psu+conv4a+mujer+pospol"# 1sem 2021
m4 <- "edhigh+psu+conv5a+mujer+pospol"# 2sem 2021
...
medconv1 <- "edhigh+psu+mujer+pospol"#highschool
medconv2 <- "edhigh+psu+mujer+pospol"# 2sem 2020
medconv3 <- "edhigh+psu+mujer+pospol"# 1sem 2021
medconv4 <- "edhigh+psu+mujer+pospol"# 2sem 2021
medconv <- c(medconv1,medconv2,medconv3,medconv4)
...
data1<- 
study_1_long %>% 
  dplyr::select(folio_encuestado,momento,vot,edhigh:conv_t1) %>%
...
voto <- 
lme4::glmer(vot~edhigh+educyear+conv_t1+mujer+pospol+momento+(1|folio_encuestado),
            data = data1,family = "binomial")
voto_re_t <- 
lme4::glmer(vot~edhigh+educyear+conv_t1+mujer+pospol+momento+(1+momento|folio_encuestado),
            data = data1,family = "binomial")
# summary(voto)

# Table ------------------------------------------------------------------------
votos <- list(votonull,voto,voto_re_t)
caption <- "Multilevel longitudinal logistic regression for voting"
knitreg(votos,
        include.ci = FALSE,
        omit.coef = c("(pospol)|(mujer)"),
