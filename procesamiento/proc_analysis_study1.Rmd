...
<!-- * Cambiar: mayor nivel educacional de cada padre reemplazar, si no hay 1, quedarse con  el que haya -->

<!-- * psu como mediador -->

<!-- * solo con conversacion 1 en voto para ver el efecto longitudinal -->
...
note <- "‘***’ p < 0.001 ‘**’ p < 0.01 ‘*’ p < 0.05 ‘.’ p < 0.1"
kable(df_mededuc,format = "html",digits = 3,col.names = colnames, 
      caption = "Mediated effect of Parental Education by PSU scores on Voting behaviour") %>% 
  kable_styling(full_width = F) %>% 
  collapse_rows(columns = 1) %>% 
  footnote(general = note,footnote_as_chunk = T)
...
data1<- 
study_1_long %>% 
  dplyr::select(folio_encuestado,vot,conv,edhigh:conv_t1) %>%
  dplyr::filter(vot !=2) %>%  
  mutate(conv_t1=as.numeric(conv_t1),
         conv=as.numeric(conv)) 
voto <- 
lme4::glmer(vot~edhigh+educyear+conv+mujer+pospol+(1|folio_encuestado),
            data = data1,family = "binomial",nAGQ = 9)
